import { ThreadChannel, GuildTextThreadCreateOptions, Collection, ChannelType, ThreadAutoArchiveDuration, REST } from 'discord.js';
...
export class ThreadManager {
  ...
  public startThreadMonitoring(): void {
    ...
    this.threadMonitoringInterval = setInterval(() => {
      this.checkThreads();
    }, typeof this.threadCheckInterval === 'number' ? this.threadCheckInterval : 60000); // Default to 60 seconds if null
  }
  ...
  public async createThread(
    channelId: string, 
    options: GuildTextThreadCreateOptions<ChannelType.PublicThread> & { message?: string, watch?: boolean }
  ...
  private async checkThreads(): Promise<void> {
    ...
    for (let i = 0; i < threadsToCheck.length; i += batchSize) {
      ...
      await Promise.all(batch.map(async ([id, threadData]) => {
   ...
   */
  public async removeThreadFromWatch(
    id: string,
    force = false
  ): Promise<boolean> {
    try {
