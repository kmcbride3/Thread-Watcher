...
function overrideWithEnv(config: ConfigFile): ConfigFile {
    const envOverrides = {
        ...
        database: {
            ...
            backupAmount: process.env.DB_BACKUP_AMOUNT ? parseInt(process.env.DB_BACKUP_AMOUNT, 10) : undefined,
    ...
    return {
        ...
        database: {
            ...
            options: {
                ...
                port: envOverrides.database.options.port !== undefined ? envOverrides.database.options.port : config.database.options.port || 3306,
                database: envOverrides.database.options.database || config.database.options.database || "threadwatcher",
                dataLocation: envOverrides.database.options.dataLocation || config.database.options.dataLocation || "./data"
            },
            backupInterval: envOverrides.database.backupInterval || config.database.backupInterval || "0 */6 * * *",
            backupAmount: envOverrides.database.backupAmount !== undefined ? envOverrides.database.backupAmount : config.database.backupAmount || 10,
            backupProvider: envOverrides.database.backupProvider || config.database.backupProvider || "none"
        },
        statsServer: {
            enabled: envOverrides.statsServer.enabled !== undefined ? envOverrides.statsServer.enabled : config.statsServer.enabled || false,
            port: envOverrides.statsServer.port !== undefined ? envOverrides.statsServer.port : config.statsServer.port || 3000
        },
        ...
        logToFile: envOverrides.logToFile !== undefined ? envOverrides.logToFile : config.logToFile || false,
        logBold: envOverrides.logBold !== undefined ? envOverrides.logBold : config.logBold || false,
        logInverted: envOverrides.logInverted !== undefined ? envOverrides.logInverted : config.logInverted || false
    };
}
...
export function getConfig(): ConfigFile {
    ...
    try {
        ...
        finalConfig.logLevel = (finalConfig.logLevel.charAt(0).toUpperCase() + finalConfig.logLevel.slice(1).toLowerCase()) as "Quiet" | "Standard" | "Debug" | "Trace";
        
        if(!finalConfig.logLevel || !["Quiet", "Standard", "Debug", "Trace"].includes(finalConfig.logLevel)) {
