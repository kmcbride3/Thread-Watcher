...
const convertToCsv = (d: ThreadData[] | ChannelData[]) =>
  d.length > 0
    ? [
        Object.keys(d[0]).join(","),
        ...d.map((item: ThreadData | ChannelData) =>
...
const reload: Command = {
  run: async (interaction: ChatInputCommandInteraction, buildBaseEmbed) => {
    ...
    const embed = buildBaseEmbed("Data export", statusType.success, {
      description: `Here's all the data saved from guild ${guildId}`,
      noSend: true,
    });
    ...
    const files: { name: string; attachment: Buffer }[] = [
      ...(threads.length > 0
        ? [
            {
              name: `threads_${guildId}.csv`,
              attachment: Buffer.from(convertToCsv(threads)),
            },
          ]
        : []),
      ...(channels.length > 0
        ? [
            {
              name: `channels_${guildId}.csv`,
              attachment: Buffer.from(convertToCsv(channels)),
            },
          ]
        : []),
    ];
    ...
    if (files.length === 0)
      buildBaseEmbed("Not found", statusType.error, {
        description: `found no data from guild "${guildId}"`,
      });
